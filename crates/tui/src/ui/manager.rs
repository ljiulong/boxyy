...
pub fn draw_manager_detail(f: &mut Frame, app: &mut App, area: Rect) {
  let columns = Layout::default()
    .direction(Direction::Horizontal)
    .constraints([Constraint::Percentage(70), Constraint::Percentage(30)])
    .split(area);

  draw_package_list(f, app, columns[0]);
  draw_package_detail(f, app, columns[1]);
}
...
fn draw_package_list(f: &mut Frame, app: &mut App, area: Rect) {
  let title = app
    ...
    .map(|name| {
      let mode_text = if supports_global(name) {
        if app.global {
          " [🌐 Global]"
        } else {
          " [📁 Local]"
      ...
      format!("{} packages{}", name, mode_text)
    })
